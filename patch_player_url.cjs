...
`  // Derive final active URL
  const activeVideoUrl = useMemo(() => {
    // 1. Check direct video sources (Languages -> Quality)
    if (videoSources && typeof videoSources === 'object') {
      const langs = Object.keys(videoSources);
      if (langs.length > 0) {
        // Try activeLanguage first, otherwise fallback to first language
