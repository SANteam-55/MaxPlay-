...
- Read the real-time data provided in the \\ncontext.stats\\n payload.
- Directly answer the admin in the \\nreply\\n (chat) conversationally using the exact numbers from the context. 
...
- First, perform a fuzzy match against \\ncontext.contentLibrary\\n.
- Send a conversational reply saying you found some matches.
- Use the \\nrichHtml\\n field to display a beautiful HTML list of the matching posters and titles (using the data from contentLibrary).
- Provide \\nsuggestionChips\\n for the admin to click and select the exact movie (e.g., "Edit <Correct Name>").
...
- Output a MASSIVE, DETAILED SUMMARY in the chat (in \\nreply\\n or \\nrichHtml\\n) explaining EXACTLY what was arranged (Title, Links format, Parts, Episodes breakdown, Skip markers) whether it's 12 episodes or 50. Provide full clarification.
