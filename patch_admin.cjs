...
const target = `       } else if (tabId === 'homerows') {         if (window.renderHomeRows) window.renderHomeRows();       }    };  </script>`;

const replacement = `       } else if (tabId === 'homerows') {         if (window.renderHomeRows) window.renderHomeRows();       } else if (tabId === 'feedback') { if (window.loadFeedback) window.loadFeedback(); }    }; 
    ...
    window.openFeedbackDetailModal = function(id) { 
      ...
      if(tk.screenshots?.length > 0) { 
        ...
        sg.innerHTML = tk.screenshots.map(s => '<img src="' + s + '" style="width:100%;aspect-ratio:16/9;object-fit:cover;" onclick="window.openFeedbackScreenshotLightbox(\\'' + s + '\\')">').join(''); 
      } else { 
