...
export const MainTabNavigator: React.FC<MainTabNavigatorProps> = ({
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-40 flex flex-col bg-[#0A0A0A]/95 backdrop-blur-2xl border-t border-[#1C1C1E] pb-[max(env(safe-area-inset-bottom,0px),8px)] select-none shadow-[0_-4px_30px_rgba(0,0,0,0.9)] md:fixed md:top-0 md:bottom-0 md:left-0 md:right-auto md:w-[80px] md:h-full md:flex-col md:justify-center md:gap-8 md:border-t-0 md:border-r md:px-0 md:py-6 md:pb-6 pointer-events-auto"
    >
      <div className="flex h-[58px] w-full max-w-lg mx-auto flex-row items-center justify-around px-2 md:h-full md:max-w-none md:flex-col md:justify-center md:gap-8 md:px-0">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <motion.button
              ...
              className={`relative flex flex-1 h-full min-h-[48px] flex-col items-center justify-center py-1 cursor-pointer outline-none transition-colors duration-200 ${
            ...
            >
              ...
              <motion.span
                ...
                transition={{ duration: 0.2 }}
                className={`mt-1 text-[11px] font-bold tracking-tight z-10 whitespace-nowrap ${
                  isActive
                    ? tab.id === 'anime'
              ...
              {isActive && (
                <motion.div
                  ...
                  className={`absolute top-0.5 md:top-1/2 md:bottom-auto md:left-0 md:h-6 md:w-1 md:-translate-y-1/2 md:rounded-r-full h-0.5 w-4 rounded-full z-20 ${
                    tab.id === 'anime'
